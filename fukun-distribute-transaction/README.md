...
![基于可靠消息服务的分布式事务](./pictures/p5.png) 
...
`"LCN并不生产事务，LCN只是本地事务的搬运工" ` ，基于切面的强一致性事务框架，
兼容 dubbo、springcloud、motan 框架，支持各种关系型数据库。  
LCN分布式事务框架其本身并不创建事务，而是基于对本地事务的协调从而达到事务一致性的效果。  
高可用，业务模块可以依赖Dubbo或SpringCloud的集群方式做集群化，TxManager也可以做集群化。  
支持本地事务和分布式事务共存，事务补偿机制，服务故障或挂机再启动时可恢复事务。  

### 核心步骤
1、创建事务组  
是指在事务发起方开始执行业务代码之前先调用TxManager创建事务组对象，然后拿到事务标示GroupId的过程。  
2、添加事务组  
添加事务组是指参与方在执行完业务方法以后，将该模块的事务信息添加通知给TxManager的操作，采用TxClient的LCN的假关闭。  
3、关闭事务组  
是指在发起方执行完业务代码以后，将发起方执行结果状态通知给TxManager的动作。当执行完关闭事务组的方法以后，TxManager将根据事务组信息来通知相应的参与模块提交或回滚事务。  

例子：  
1、事务的发起方与参与方必须都要注册到事务协调者中，事务的发起方与参与方都跟TxManager事务协调器建立长连接。  
...
4、如果库存服务获取到请求头中有对应的事务分组id，库存服务业务逻辑代码执行完毕后，会采用LCN假关闭，不会提交该事务。  
注意：TxClient的代理连接池实现了Javax.sql.DataSource接口，并重写了close方法，事务模块在提交关闭后，TxClient连接池将执行‘假关闭’操作，等待TxManager协调完成事务后再关闭连接。  
微服务里面的服务和服务之间 协调在一起 通过注册中心，比如consul、eureka等， 解决事务和事务之间的关系  靠的是类似的平台  TxManager  去协调事务，就是加入到同一个事务组中。  
5、参与方成功执行相关的业务代码之后，前提是订单服务（发起方）调用库存服务接口（参与方）之后，如果订单服务（发起方）执行没有问题的话，就是说
订单服务（发起方）使用对应的事务分组id，将结果通知给TxManager事务协调者，然后TxManager事务协调者再次根据该事务分组id，通知给所有的参与方真正的提交事务。    

注意：需要redis，事务分组ID都是缓存到Redis中的，如果参与方在一定的时间内没有收到提交事务的通知会自动进行回滚本地事务。  
### 模拟场景演示
若存在事务发起方、参与方A、参与方B。调用关系图如下： 
 
![LCN的分布式事务机制](./pictures/p9.png)  

正常的执行时序如下：  

![LCN的分布式事务机制](./pictures/p10.png)  

如果参与方B出现异常，时序图如下：  

![LCN的分布式事务机制](./pictures/p11.png)  

如果调用关系是下面的形式，如下：  

![LCN的分布式事务机制](./pictures/p12.png)    

此时发生参与方B出现异常时他们的时序图为：  

![LCN的分布式事务机制](./pictures/p13.png)     
